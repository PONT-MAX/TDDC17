...
(define (domain logistics)
  ...
  (:action load
    :parameters (?o ?v ?l)
    ;; Either all vehicle can pick up objects
    ;; or big truck/train/plane picks up big objects.
    :precondition (or (and (object ?o) (vehicle ?v) (location ?l)
		       (at ?v ?l) (at ?o ?l))
		      (and (bigobject ?o) (location ?l)
		       	   (or(bigtruck ?v) (train ?v) (airplane ?v))
		       (at ?v ?l) (at ?o ?l)))
    :effect (and (in ?o ?v) (not (at ?o ?l))))

  (:action unload
    :parameters (?o ?v ?l)
    :precondition (and (or (bigobject ?o) (object ?o)) (vehicle ?v) (location ?l)
		       (at ?v ?l) (in ?o ?v))
  ...
  (:action drive
    ...
    :precondition (and (or (truck ?t) (bigtruck ?t)) (location ?l1) (location ?l2) (city ?c)
